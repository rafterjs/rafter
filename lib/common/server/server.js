...
export default (
  ...
  async function initPreStartHooks() {
    if (preStartHooks.length > 0) {
      ...
      const hooks = preStartHookProvider.createInstance(preStartHooks);
  ...
  async function initMiddleware() {
    if (middlewareConfig.length > 0) {
      express.use(middlewareProvider.createInstance(middlewareConfig));
    }
  }
  ...
  async function initRoutes() {
    if (routesConfig.length > 0) {
      express.use(routerProvider.createInstance(routesConfig));
    }
  }
  ...
  Server.start = async () => {
    if (!serverInstance) {
      ...
      return new Promise((resolve, reject) => {
        serverInstance = express.listen(config.server.port, error => {
          if (error) {
            logger.error(error);
            reject(error);
          }

          logger.info(`ExpressServer::start Server running on port ${config.server.port}`);
          resolve();
        });
      });
    }

    logger.warn(`ExpressServer::start Server is already running on port ${config.server.port}`);
